...
class Bank {
  ...
  constructor(box, oracle) {
    this.box = wasm.ErgoBox.from_json(box);
    this.oracle = oracle;
  }
  ...
  able_to_mint_stable_coin = (amount) => {
  ...
  mint_stable_coin_reserve_ratio = (amount) => {
  ...
  able_to_mint_reserve_coin_amount = (amount) => {
  ...
  mint_reserve_coin_reserve_ratio = (amount) => {
    const new_base_reserve =
  ...
  total_cost_to_mint_stable_coin = (amount, transaction_fee) => {
  ...
  fees_from_minting_reserve_coin = (amount, transaction_fee) => {
    const fee_less_amount = this.reserve_coin_nominal_price() * amount;
    const protocol_fee = parameters.PROTOCOL_FEE(fee_less_amount);
    return transaction_fee + protocol_fee;
  };

  base_cost_to_mint_reserve_coin = (amount) => {
    const fee_less_amount = this.reserve_coin_nominal_price() * amount;
    const protocol_fee = parameters.PROTOCOL_FEE(fee_less_amount);
    return fee_less_amount + protocol_fee;
  };

  base_amount_from_redeeming_reserve_coin = (amount) => {
    const fee_less_amount = this.reserve_coin_nominal_price() * amount;
    const protocol_fee = parameters.PROTOCOL_FEE(fee_less_amount);
    return fee_less_amount - protocol_fee;
  };

  base_amount_from_redeeming_stable_coin = (amount) => {
    const fee_less_amount = this.stable_coin_nominal_price() * amount;
    const protocol_fee = parameters.PROTOCOL_FEE(fee_less_amount);
    return fee_less_amount - protocol_fee;
  };

  create_candidate = (height, adding_stable_count, adding_reserve_count) => {
